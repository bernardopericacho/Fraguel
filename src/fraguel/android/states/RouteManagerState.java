...
public class RouteManagerState extends State {
	...
	public void load() {
		...
		container = new LinearLayout(FRAGUEL.getInstance()
	...
	private void loadPoints(int route) {
		...
		title.setText(title.getText() + " '"
				+ FRAGUEL.getInstance().routes.get(route).name + "'");
		...
		if (urls.length != 0) {
			this.imageThread = new ImageDownloadingThread(urls, names,
					ResourceManager.getInstance().getRootPath() + "/tmp/route"
							+ FRAGUEL.getInstance().routes.get(route).id + "/");
			imageThread.start();
		}
		internalState = 1;
	...
	private void setAdapter() {
		...
		adapter = new RouteManagerAdapter(FRAGUEL.getInstance()
				.getApplicationContext());
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		if (item.getGroupId() == 0)
			deleteSelectedRoute(FRAGUEL.getInstance().routes.get(item
					.getItemId()).id);
		else {
			if (item.getItemId() == 0) {
				this.routeInfo = new RouteInfoDialog(FRAGUEL.getInstance(),
						route, false);
				routeInfo.show();
			} else
	...
	public void AllAvailableRoutes() {
		...
		FRAGUEL.getInstance().createDialog("Rutas disponibles", options,
